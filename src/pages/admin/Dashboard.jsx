...
export default function DashboardAdmin() {
  ...
  const queryParamsGrafik = useMemo(
    ...
    [page, LIMIT, province, regency]
  );

  const { data: dataThisDay } = useGetReports(queryParams);
  const { data: graphicData } = useGetReports(queryParamsGrafik);
  ...
  if (graphic.length) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      chartData.push({ date: dateStr, count: grouped[dateStr] || 0 });
    }
  }

  // Statistik
  const pendingCount = datasThisDay.filter(
    (r) => r.verification_status === "PENDING"
  ).length;

  const completeReportProgressCount = datasThisDay.reduce(
    (total, r) => total + r.progressUpdates.filter((pg) => pg.stage === "COMPLETED").length,
    0
  );
  const verifiedReportProgressCount = datasThisDay.reduce(
    (total, r) => total + r.progressUpdates.filter((pg) => pg.stage === "REVIEW").length,
    0
  );
  const inprogressReportProgressCount = datasThisDay.reduce(
    (total, r) => total + r.progressUpdates.filter((pg) => pg.stage === "INPROGRESS").length,
    0
  );

  return (
    <div className="container mx-auto px-4 lg:pl-40 lg:pr-52 2xl:pl-60 2xl:pr-60 2xl:m-12">
      <div className="flex items-stretch gap-4">
        ...
        <div className="mr-6">
          <div className="flex justify-between gap-6">
            <div>
              ...
              <p className="w-52 h-28 bg-white rounded-md flex items-center justify-center mt-3 font-extrabold text-6xl">
                {datasThisDay.length}
              </p>
            </div>
            <div>
              ...
              graphic.length ? (
                <div className="w-full h-full flex justify-start flex-col py-3 pr-6">
                  <div className="w-full flex justify-center gap-2 mb-2">
                    ...
                    >
                      <img src={TagClose} className="w-5" />
                    </ButtonSubmit>
                ...
                <div className="flex w-full h-full flex-col bg-white rounded-md p-4">
                  ...
                  >
                    <img src={TagClose} className="w-7" />
                  </ButtonSubmit>
        ...
        <div className="w-3/4 flex flex-col items-start mt-3">
          ...
          <div className="w-full flex flex-col items-start mt-2 h-full">
            {pendingCount === 0 && datasThisDay.length === 0 ? (
