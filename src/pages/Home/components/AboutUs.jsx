    ...
    >
      ...
      <div className="mx-auto h-full w-full">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          ...
          <div className="mt-4 grid max-w-6xl grid-cols-4 divide-x divide-black/10 overflow-hidden rounded-xl border border-white/15 bg-[#F7EEDF] backdrop-blur-sm">
            {stats.map((stat, index) => (
              <div
                key={index}
                className="flex flex-col items-center justify-center space-y-3 px-6 py-8 text-center"
              >
                <h1 className="text-6xl font-[500]">{stat.value}</h1>
                <h2 className="text-lg font-[400]">{stat.label}</h2>
              </div>
            ))}
          </div>
        </div>
